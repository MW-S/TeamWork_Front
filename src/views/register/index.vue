<template>
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" autocomplete="on" label-position="left" label-width="50px">
      ...
      <el-form-item prop="phone" label="手机 :" label-width="60px">
        <el-input
          ref="phone"
...
export default {
  ...
  data() {
    const validateGender = (rule, value, callback) => {
      if (value.length < 0) {
        callback(new Error('The gender can not be null'))
      } else {
        callback()
      }
    }
    const validatePhone = (rule, value, callback) => {
      if (value.length < 11) {
        callback(new Error('The Phone can not be less than 11 digits'))
      } else {
        callback()
      }
    }
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('user/register', this.loginForm)
            .then(response => {
              this.$message.success('您的注册成功！您的账号为：' + response.username)
              setTimeout(() => {
